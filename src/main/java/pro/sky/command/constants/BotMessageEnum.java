package pro.sky.command.constants;
/**
 * Класс перечисление. Для кнопок клавиатур бота.
 * содержит 2 финальных поля и конструктор. Для указания сообщения по нажатию кнопки и ее названия
 * @autor Шилова Наталья
 */
public enum BotMessageEnum {

    //ответы на команды с клавиатуры
    HELP_MESSAGE("""

              Я бот Приюта для животных. Я расскажу Вам о приюте или помогу связаться с волонтерами.
              
                Помните выбрать любимца нельзя по фото, а можно только при личном общении.
                 Поэтому наш бот не сможет заменить визит в питомник, а только лишь поможет узнать о приюте и его правилах.

            """, ""),

    START(" \"✅ Для дальнейшей работы выберите  хотите вы узнать о приюте кошек или собак.\n\n" +
                      "Воспользуйтесь клавиатурой, чтобы начать  \uD83D\uDC47", "Главное меню"),

    CALL_VOLUNTEER("Ваш запрос важен для нас.\n" +
            "Напишите сообщением свой вопрос волонтеру."
            , "Позвать волонтера"),

    //первое меню
    CAT_SHELTER("кошачий приют ", "кошачий приют "),
    DOG_SHELTER("собачий приют ", "собачий приют"),


    //второе меню

    INFO("Вы можете почитать про наш приют", "Инфо"),
    //при отправке команды в чат сразу послать сообщение с краткой инфой о приюте и к нему приделать третье меню
    TAKE_PET(" Бот поможет потенциальным усыновителям животного из приюта разобраться с бюрократическими" +
            " и бытовыми вопросам. Нажмите кнопку, чтоб получить более подробную информацию.",
            "Взять питомца."),
   TEST_PERIOD("Вы обязаны присылать правильно заполненный отчет каждый день. " +
           "Чтоб пройти испытательный срок вы должны прислать 15 правильно заполненных и подтвержденных отчетов. " +
           "Испытательный срок может быть продлен на усмотрение волонтера на 10 дополнительных отчетов.\n\n " +
           "Если вы не присылали отчет более 2х дней в 12.00(по времени сервера)  вам будет приходить сообщение о том что нужно заполнить отчет.\n\n" +
           "Так же вам будут приходить сообщения о утверждении или отклонении присланного отчета после проверки их волонтерами. \n\n" +
           "Пожалуйста примите во внимание что волонтеры работают на добровольных началах в свободное время и оповещения могут приходить в разное время." +
           "если вас это напрягает отключте звук этого чата, но не забывайте во время присылать отчеты" ,"Испытательный срок"),
    PET_REPORT(" Вы обязаны присылать информацию о том, как животное чувствует себя на новом месте. \n" +
            "В ежедневный отчет входит следующая информация:\n" +
            "\n" +
            "- *Фото животного*\n" +
            "- *Рацион животного*\n" +
            "- *Общее самочувствие и привыкание к новому месту*\n" +
            "- *Изменение в поведении: отказ от старых привычек, приобретение новых*", "Отчет о питомце"),

    //меню информации о приюте
    SHELTER_INFO("Вы можете почитать более подробную информацию о нашем приюте.", "Подробная информация"),
    SHELTER_DATA("вы можете посмотреть график работы и как нас найти", "график  работы и контакты"),
    TAKE_PASS("Напишите в чат сообщение где будет содержаться краткая информация о транспортном средстве. Волонтер свяжется с вами если информация будет не полной.\n" +
            "Если вы хотите запросить звонок отошлите в чат номер телефона\n без специальных знаков. Просто 11 цифр начиная с 8\n\n" +
            "Не забудьте ознакомится с правилами безопасности на территории", "Запросить пропуск"),
    RULES_SAFETY(" Общие рекомендации о технике безопасности на территории приюта ", "Правила безопасности"),
    TAKE_DATA_FOR_CONTACT("Вы можете оставить мобильный телефон для связи. Сообщение необходимо оставить в следующем виде:\n\n" +
            "номер телефона без специальных знаков. Просто 11 цифр начиная с 8 ", "Оставить данные для связи"),

    //Меню взять питомца
    RULES_SHELTER("правила знакомства с животным до того, как забрать его из приюта", "Правила"),
    DOCUMENTS("Здесь перечень документов, необходимых для того, чтобы взять животное из приюта", "Документы"),
    TRANSPORTATION("Правила транспортировки", "Рекомендации по транспортировке"),
    SETTLE("Здесь правила содержания", "Правила содержания животного"),
    //Добавить кнопки и содержанием маленького, взрослого, с ограничениями
    LITTLE_PET("Правила содержания малышей", "Малыши"),
    BIG_PET("Правила содержания взрослых животных", "взрослые животные"),
    DISABILITY_PET("Правила содержания животных с ограничениями", "животные c ограниченными возможностями"),

    DOG_HANDLERS("Здесь список кинологов", "Кинологи"),
    //добавить кнопки первичное обращение и для потоянного наблюления
    REASONS_REFUSAL("Здесь какие то причины отказа", "Причины Отказа"),

    //меню отчета
    PHOTO("Вы должны прислать фото питомца. В подписи к фото вы должны указать дату отчета и номер питомца из договора в формате:\n\n гггг-мм-дд 00000 текст отчета ", "Фото питомца"),
    DIET("Вы должны прислать рацион питомца. Отчет можно прислать только после нажатия соответствующей кнопки." +
            " Текст сообщения начните с даты и номера питомца из договора в формате:\n\n гггг-мм-дд 00000 текст отчета", "Рацион питомца"),
    HEALTH("Вы должны прислать отчет о здоровье питомца. Отчет можно прислать только после нажатия соответствующей кнопки." +
            " Текст сообщения начните с даты и номера питомца из договора в формате:\n\n гггг-мм-дд 00000 текст отчета", "Здоровье питомца"),
    BEHAVIOR("Вы должны прислать изменение привычек питомца. Отчет можно прислать только после нажатия соответствующей кнопки." +
            " Текст сообщения начните с даты и номера питомца из договора в формате:\n\n гггг-мм-дд 00000 текст отчета", "Изменение привычек");
    private final String message;

    private final String nameButton;

    BotMessageEnum(String message, String name) {
        this.message = message;
        this.nameButton = name;
    }

    public String getMessage() {
        return message;
    }

    public String getNameButton() {
        return nameButton;
    }
}
